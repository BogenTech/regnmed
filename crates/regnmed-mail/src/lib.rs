...
//!
//! Its own crate (#75) because BOTH binaries publish: the API sends on
//! human actions, the CLI's abonnement automation sends from CronJobs —
//! and a wire contract must have exactly one copy.
//!
...
// Re-exported so callers can name the context type without their own
